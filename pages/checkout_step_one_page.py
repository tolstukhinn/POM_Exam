...
class CheckoutStepOne(BasePage):
    ...
    def data_entry(self,first_name,last_name,zip_code):
        ...
        self.wait.until(EC.element_to_be_clickable(self._FIRST_NAME_FIELD)).clear()
        self.wait.until(EC.element_to_be_clickable(self._FIRST_NAME_FIELD)).send_keys(first_name)

        self.wait.until(EC.element_to_be_clickable(self._LAST_NAME_FIELD)).clear()
        self.wait.until(EC.element_to_be_clickable(self._LAST_NAME_FIELD)).send_keys(last_name)

        self.wait.until(EC.element_to_be_clickable(self._ZIP_FIELD)).send_keys(Keys.CONTROL + "A")
        self.wait.until(EC.element_to_be_clickable(self._ZIP_FIELD)).send_keys(Keys.BACKSPACE)
